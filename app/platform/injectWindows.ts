...
export async function injectTextWindows(text: string): Promise<InjectResult> {
  return new Promise((resolve) => {
    ...
    // Wait for target app to regain focus after our window hides
    setTimeout(() => {
      ...
      // PowerShell command - ONE paste operation, NO loops
      // 1. Decode Base64 to get original text (prevents injection)
      // 2. Add-Type loads Windows Forms for SendKeys
      // 3. Set-Clipboard puts text on clipboard
      // 4. Small sleep ensures clipboard is ready
      // 5. SendKeys ^v = Ctrl+V = Paste
      const command = `$text = [System.Text.Encoding]::Unicode.GetString([Convert]::FromBase64String('${encodedText}')); Add-Type -AssemblyName System.Windows.Forms; Set-Clipboard -Value $text; Start-Sleep -Milliseconds 100; [System.Windows.Forms.SendKeys]::SendWait('^v')`
